...
export default function RecipeDetailPage() {
  ...
  if (!recipe) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-4 text-center">
        <p className="font-semibold text-navy">레시피를 찾을 수 없어요</p>
    ...
    return (
      <div className="mx-auto flex max-w-md flex-col" style={{ minHeight: 'calc(100vh - 80px)' }}>
        <div className="flex items-center justify-between px-4 py-4">
          <button onClick={() => setCookingMode(false)} className="flex h-9 w-9 items-center justify-center rounded-full bg-white shadow-sm">
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
          <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-mint text-2xl font-bold text-white shadow-lg">
            {currentStep + 1}
          </div>
          ...
          {step.time && (
            <div className="mt-4 flex items-center gap-2 rounded-full bg-mint-light px-4 py-2">
        ...
        <div className="px-4 pb-6">
          <div className="mb-3 flex gap-2">
            {currentStep > 0 && (
              <Button onClick={() => setCurrentStep((s) => s - 1)} variant="outline" className="flex-1 rounded-xl py-5">
                이전
              </Button>
            )}
            {currentStep < recipe.steps.length - 1 ? (
              <Button onClick={() => setCurrentStep((s) => s + 1)} className="flex-1 rounded-xl bg-mint py-5 text-white hover:bg-mint-dark">
                다음 단계
              </Button>
            ) : (
              <Button onClick={() => setCookingMode(false)} className="flex-1 rounded-xl bg-freshness-fresh py-5 text-white">
                <CheckCircle className="mr-2 h-5 w-5" /> 요리 완성!
              </Button>
            )}
          </div>
  ...
  return (
    <div className="mx-auto max-w-md">
      ...
      <div className="px-4 pb-6">
        {/* Hero */}
        <div className="mb-4 rounded-2xl bg-gradient-to-br from-navy to-navy-light p-6 text-white">
          ...
          {recipe.description && <p className="mb-4 text-sm text-white/70">{recipe.description}</p>}
        ...
        {recipe.nutrition && (
          <div className="mb-4 rounded-2xl bg-white p-4 shadow-sm">
            <p className="mb-3 font-semibold text-navy">영양 정보 (1인분)</p>
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <div key={label} className="flex flex-col items-center rounded-xl bg-gray-50 p-2">
        ...
        <div className="mb-4 rounded-2xl bg-white p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <p className="font-semibold text-navy">재료 ({recipe.ingredients.length}개)</p>
            {missingIngredients.length > 0 && (
              <button onClick={handleAddAllMissing} className="flex items-center gap-1 rounded-full bg-accent-blue/10 px-3 py-1 text-xs font-medium text-accent-blue">
            ...
            {recipe.ingredients.map((ing) => (
              <div key={ing.name} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`h-2 w-2 rounded-full ${ing.inFridge ? 'bg-freshness-fresh' : 'bg-gray-300'}`} />
                  <span className={`text-sm ${ing.inFridge ? 'text-navy' : 'text-gray-400'}`}>{ing.name}</span>
                  {ing.inFridge && <span className="rounded-full bg-mint-light px-1.5 py-0.5 text-[10px] text-mint">냉장고</span>}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {!ing.inFridge && ing.required && (
                    <button
                      ...
                      className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${addedItems.has(ing.name) ? 'bg-mint-light text-mint' : 'bg-accent-blue/10 text-accent-blue hover:bg-accent-blue/20'}`}
                    >
        ...
        {/* Steps */}
        <div className="mb-4 rounded-2xl bg-white p-4 shadow-sm">
          <p className="mb-3 font-semibold text-navy">조리 순서</p>
          <div className="flex flex-col gap-4">
            {recipe.steps.map((step) => (
              <div key={step.order} className="flex gap-3">
                <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-mint text-sm font-bold text-white">
                  {step.order}
                </div>
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          {recipe.tags.map((tag) => (
            <span key={tag} className="rounded-full bg-gray-100 px-3 py-1 text-sm text-gray-500">#{tag}</span>
          ))}
        </div>
